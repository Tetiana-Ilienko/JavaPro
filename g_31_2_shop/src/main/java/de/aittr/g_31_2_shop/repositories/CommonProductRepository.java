...
public class CommonProductRepository implements ProductRepository {
    ...
    public Product save(Product product) {
        try (Connection connection = getConnection()) {
            String query = String.format(Locale.US, "INSERT INTO `31_2_shop`.`product` (`name`, `price`, `is_active`)" +
                    " VALUES ('%s', '%.2f', '1');", product.getName(), product.getPrice());
            connection.createStatement().execute(query);

            query = "select id from 31_2_shop.product order by id desc limit 1;";
    ...
    public List<Product> getAll() {
        try (Connection connection = getConnection()) {
            String query = "select id, name, price from 31_2_shop.product where is_active = 1;";
            ResultSet resultSet = connection.createStatement().executeQuery(query);
